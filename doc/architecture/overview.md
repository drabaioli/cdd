...
A third guard of the same family, `scripts/roadmap-length-check.sh` (the `roadmap-length` gate), holds every checkbox item in `doc/knowledge_base/roadmap.md` to 200 characters — pending items as much as completed ones. The roadmap is loaded by every session, so an over-long item is a context cost paid forever; the bar had been stated in the roadmap's own conventions section for a long time and had not held, because the section shipped an escape clause grandfathering existing violations. See ADR 0005 for the decision and for where detail goes instead. Two mechanics are worth knowing. It is scoped to that one file on purpose: `template/doc/knowledge_base/roadmap.md` and the demo seed's roadmap are instructional prose for a human bootstrapping a project, not progress-log lines, and are legitimately longer. And it counts characters portably — `awk`'s `length()` is byte-based in mawk and character-based in gawk under a UTF-8 locale, which would give two contributors different verdicts and would silently charge 3 of the 200 for an em dash, the punctuation the convention actually mandates — so the scan runs under `LC_ALL=C` and strips UTF-8 continuation bytes first. Rather than a sibling mutation harness it carries an **inline self-check**: three fixtures (one character under the cap, one over, and a multibyte line at exactly the cap in characters but over it in bytes) must yield exactly one hit before the real scan is trusted, so both realistic silent-failure modes — the item pattern stops matching, character counting regresses to bytes — are pinned without an eighteenth gate.
