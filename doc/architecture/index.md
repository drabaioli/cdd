...
- [Capability adapters](capability-adapters.md) — the wire contract for `.cdd/` adapters, pinned for the tracker capability: the six verbs and their JSON shapes, the exit-code table, `describe` and its `contract` versioning rule, the GitHub reference adapter, the announcement rule, and what the offline conformance gate does and does not prove
