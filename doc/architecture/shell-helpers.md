...
`cdd-state issue-refs <branch> <ref>...` records the issue reference(s) an issue-driven task was sourced from, as a JSON array of strings under `issue_refs`. The branch is positional for the same reason `lane`'s is — `/cdd-next-step` writes it while standing on the default branch — and it is its own subcommand for the same failure-isolation reason: an older helper fails this one call and keeps the seeded record, and the task falls back to the branch token. It appends no session entry and writes no per-repo marker, and it refuses an option-shaped branch or ref, and an empty ref list, with exit 2 and no write. Where `lane` and `base_branch` are write-once, this field is **amendable**: each call replaces the whole list, so a task that grows a second issue re-runs the verb with the full set rather than needing an "add" verb. The refs are passed through verbatim — normalizing one is the tracker adapter's job (§2.16) — and `jq --args` puts them into the array without quoting games. `/cdd-pre-pr` reads them back through `cdd-state get issue_refs`.

`cdd-state set-field <x-key> <json-value> [--branch <branch>]` writes one **extension field** (process doc §2.13). The key must start with `x-` and anything else exits 2 without writing: that guard is not decoration, it is what stops the verb being a backdoor around `set`'s stage-enum validation (`set-field stage merged`), and CDD's own future fields get their own subcommand the way `lane` did, so nothing is lost by it. The value is JSON — object, array or scalar — validated *before* the file is touched, so an invalid one exits 2 with the record byte-identical; `null` is a legitimate value and lands as a present, null-valued field, since absent and null already mean different things here (`pr`, `lane`). Removing a field is deliberately not in scope. The branch defaults to the current worktree's (as `set` does) and `--branch` names another, for a caller standing on the default branch; it is a flag rather than a positional because the derived case is the common one. Like `lane` it appends no session entry and writes no per-repo marker — it annotates a task rather than advancing it — and like every other write it is atomic and syncs to the task ref. An absent record is not fabricated: it prints the advisory line and exits 0.

What makes the namespace usable is that no verb rewrites the record wholesale: `set`, `lane`, `issue-refs` and `set-field` each build a `jq` filter of field *assignments* over the existing file, so unknown top-level keys — an extension's, or a newer CDD's — ride through untouched, and `cdd-state get <field>` reads `.[$f]` generically, so reading one needs no code at all. That property is invisible until it breaks, so `scripts/state-extension-assert.sh` pins it against behaviour rather than against the current filters: a refactor of any verb into a rebuild-from-scratch form fails the `state-extension` gate.
...
## Reading the handoff directory (`cdd-worktree-handoff-branches`)

Every task artifact in `~/.cdd/handoffs/<repo>/` is a flat, branch-named sibling: `<branch>.md`, `<branch>.plan.md`, `<branch>.state.json` (plus the non-task `repo.json`). The plan's extension is `.md`, the same as the handoff's, so a bare `*.md` glob matches both and `basename "$f" .md` would yield a phantom task named `<branch>.plan` — the hazard `.state.json` avoids only by having a different extension.

`cdd-worktree-handoff-branches` is the single place that knows this: it globs `*.md`, skips the branch-named sidecars, and prints one task branch per line. `cdd-worktree-list` and `cdd-worktree-gc` both go through it rather than each carrying its own filter, so a future sidecar of the same shape is one line here and nothing anywhere else. The one consumer that cannot call it is `/cdd-next-step`'s stale-handoff scan, which is a prompt rather than shell: it carries the same filter in prose. `scripts/gc-assert.sh` pins the property from the outside — a plan file present, and the listing still shows exactly one row.
...
**What it enumerates and removes.** Candidates are the union of local handoff/state basenames in `~/.cdd/handoffs/<repo>/` and the `refs/cdd/*` names from `git ls-remote origin` — so a machine that never held a task's files can still reap that task's leaked remote ref, and a machine holding orphaned local copies can reap them even after the ref is already gone. For each merged candidate it removes whichever of the four artifacts are present (local `<branch>.md`, `<branch>.plan.md`, `<branch>.state.json`, remote ref). It does not glob for plan files: a task with a plan always has a handoff, so the shared enumerator above already names it (both the reap and the no-phantom-row property are pinned by `scripts/gc-assert.sh`). It is **dry-run by default** — printing `reap … would remove …` / `keep …` lines — and only deletes under `--force`. Being conservative and idempotent, it is safe to run (or schedule) routinely.
