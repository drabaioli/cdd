...
`cdd-worktree-handoff-branches` is the single place that knows this: it globs `*.md`, skips the branch-named sidecars, and prints one task branch per line. `cdd-worktree-list` and `cdd-worktree-gc` both go through it rather than each carrying its own filter, so a future sidecar of the same shape is one line here and nothing anywhere else. `scripts/gc-assert.sh` pins the property from the outside — a plan file present, and the listing still shows exactly one row.
