...
- [x] Per-task state record + `cdd-state` helper: a `<branch>.state.json` sibling of the handoff, seeded by `/cdd-next-step` and advanced by the implementation session and the `/cdd-merge-base` / `/cdd-pre-pr` / `/cdd-process-pr` commands; deleted alongside the handoff by `cdd-worktree-done`. Advisory, local-only, append-only `{id, stage}` session chain (resume command derived as `claude --resume <id>`). Writes go through `tools/cdd-state.sh` (dual-mode, self-installing like `cdd-worktree.sh`) — atomic `seed` / `set` instead of hand-edited JSON, so the malformed-JSON / wrong-field failure mode is gone; the model still decides *when*. Single `stage` enum (no separate `status`); no timestamps / `machine` / `url` fields (dropped as speculative). — process doc §2.13 (rewritten) + §2.6/§2.8/§3.3 + all four command copies (repo + template) + both `settings.json` (`cdd-state`/`rm *.state.json` allows) + `tools/cdd-state.sh` (new) + `tools/cdd-worktree.sh` (deletion) + architecture/feature docs + BOOTSTRAP.md.
- [ ] Harden the one outcome transition a tool call owns: a `PostToolUse` hook matching `gh pr create` that parses the PR number from the tool output and writes `pr_open` / `pr=NN` mechanically (`cdd-state` can be the hook target), so it no longer depends on the model remembering. Design note: a *fully guaranteed* trigger exists (a `UserPromptSubmit`/`UserPromptExpansion` hook matched on the command name fires deterministically on every `/cdd-*` invocation), but it fires at command **invocation**, so it can only stamp a coarse "stage started" — it cannot capture **outcomes** (`checks_passed`, the PR number) the model only produces mid-run. Other outcome transitions stay model-driven via `cdd-state set`; verify `UserPromptExpansion` exists in the target Claude Code version before relying on it (`UserPromptSubmit` is the stable one).
