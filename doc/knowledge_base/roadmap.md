...
- [x] Improve the placeholder-substitution recipe in the template README; renamed to `template/BOOTSTRAP.md`, with the sed recipe replaced by `bootstrap-cdd-project.sh`
...
- [x] rc-install in demo setup/teardown: `demo/setup.sh` appends a marker-guarded sourcing block to `~/.bashrc`, and `demo/teardown.sh` removes it by marker; mdr_demo_01 friction round
...
- [x] ADRs: Nygard-style `doc/architecture/adr/NNNN-title.md`, shipped in both the template and the CDD repo, referenced from the process doc and CLAUDE.md; mdr_demo_01 friction round
...
- [x] Trial the retrofit on one existing project; Colibri (Zephyr/C++), which surfaced the change-isolation defect below.
...
- [x] Add a standard MIT `LICENSE` at the repo root (holder: Diego Andres Rabaioli, 2026); repo-root only, since the template is copied verbatim into downstream projects
...
- [x] `SECURITY.md`: a repo-root file directing reporters to GitHub private vulnerability reporting, not email; assumes the repo setting is enabled (a manual one-time toggle)
...
- [x] `cdd-state set-field` plus the `x-` extension-field convention, with a gate asserting unknown top-level keys survive a write.
- [x] Tracker verb contract + GitHub reference adapter; 6 verbs, `describe`, JSON shapes, exit codes, plus a conformance gate.
- [x] Teach `/cdd-next-step` the resolution ladder and `ref_pattern` dispatch, replacing the hardcoded `#NN` front-end; `gh` stays the fallback.
- [x] Move issue refs from the branch name onto the state record (multi-ref, any backend); `/cdd-pre-pr` emits one close line per ref via `issue-close-token`, `gh_issue_NN_` kept as fallback (#97).
