...
- [x] Reinforce objective 2 at bootstrap: a required bootstrap-phase task and/or checklist, once the `/cdd-pre-pr` mechanism is proven. — `/cdd-bootstrap` now asks for the engineering floor in discovery, overlays the contract resolved rather than as a skeleton, and derives the roadmap's infrastructure tasks from its *expected* rows. Guided path only; `/cdd-retrofit` install mode stays exempt.
