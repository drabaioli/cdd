...
- [x] Objective-3 standing channel: a recurring mechanism that routes a discovered improvement into the roadmap/conventions (not a reintroduced standing log). — landed as `/cdd-pre-pr` step 7, "Workflow improvement check (conditional)", in both command copies: six concrete observable triggers, three routes by scope (apply the edit now / propose a roadmap item / offer a human-gated `gh issue create --repo drabaioli/cdd` with the roadmap item as fallback), default silence, non-blocking, not a seventh checkpoint. Prompt-only — no script, gate, or seam, matching the CI-improvement and test-coverage steps. No new artifact, so the retired friction log stays retired. Complements `/cdd-retrofit` upgrade mode (discovery time vs. upgrade time) rather than duplicating it. Renumbered steps 7–10 → 8–11, which propagated to `prompt-seam-check.sh`'s pinned heading list and the check-4 mutation case in `prompt-seam-assert.sh`, and fixed a pre-existing stale "step 8" reference to the auto-commit. Process doc §1 (commitment 5 now names both channels), §3.5 ("three things" → four), §6 (deferred gap removed, retrofit bullet re-scoped). Design and rejected alternatives in ADR [`0003-standing-self-improvement-channel.md`](../architecture/adr/0003-standing-self-improvement-channel.md).
