...
- [x] Add a GitHub-issue front-end to `/next-step`: an issue number (`#123`/bare integer) scopes that issue directly; a reserved `issue`/`issues` keyword lists open issues (excluding those with an existing worktree/PR) and lets the user pick. The issue body+comments seed the same intent-driven machinery (adaptive context load, overlap check, roadmap-belonging decision). The picked branch is named `gh_issue_NN_<slug>` so the issue number is durable. As a general enhancement, `/pre-pr` gains an opt-in "open the PR?" step; when the branch matches `gh_issue_NN`, the PR body gets `Closes #NN`. — process doc §3.1/§3.5/§3.6 + both `next-step.md` + both `pre-pr.md` copies + both CLAUDE.md workflow bullets.
