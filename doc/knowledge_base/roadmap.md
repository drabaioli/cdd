...
- [ ] Harden writes from best-effort toward reliable. The current writes are prompt-driven (each command instructs Claude to edit the JSON), so they are advisory by design. Two enhancements, in order of value: **(a)** a dual-mode `cdd-state` helper (sourced/explicit like `cdd-worktree.sh`) that does atomic, correct writes — the prompts call `cdd-state set <stage> <status>` instead of hand-editing JSON, killing the malformed-JSON / wrong-field failure mode while the model still decides *when*; **(b)** a `PostToolUse` hook matching `gh pr create` that parses the PR number from the tool output and writes `pr_open`/`pr=NN` mechanically — the one transition whose data lives in a concrete tool call (`cdd-state hook` can be the hook target). Design note: a *fully guaranteed* trigger exists (a `UserPromptSubmit`/`UserPromptExpansion` hook matched on the command name fires deterministically, harness-run, on every `/cdd-*` invocation), but it fires at command **invocation**, so it can only stamp a coarse "stage started" — it cannot capture **outcomes** (`checks_passed`, the PR number) the model only produces mid-run. Outcome transitions stay model-driven or ride a tool-specific `PostToolUse` hook; verify `UserPromptExpansion` exists in the target Claude Code version before relying on it (`UserPromptSubmit` is the stable one).
