...
- `./scripts/adapter-conformance-check.sh` — each shipped capability adapter (the gate runs it over every `tools/adapters/*/*.sh`; the capability named by the subject's `describe` picks the contract verbs and probes) against the contract in `doc/architecture/capability-adapters.md`: `describe` is hermetic and contract-shaped, every verb it declares dispatches to a real implementation, an undeclared verb exits 3, a usage error exits 2, missing backend tooling or configuration exits 4 with an actionable line, and nothing secret-shaped is committed alongside it. Offline by construction — it runs the adapter with the environment scrubbed, under a scratch `PATH` holding stub backend tools (`gh`, `curl`) or none at all, so no probe-mode flag has to exist in the contract for the gate's benefit. Its verb probe proves dispatch *reaches* an implementation, not that the implementation is correct; that limit is stated in the contract doc rather than left for a reader to discover.
- `./scripts/adapter-conformance-assert.sh` — the conformance checker's own contract, by mutation: twenty-one broken adapters, one defect each, every one of which the checker must *fail* on and name — including a Jira one that exports fake credentials to prove the environment scrub is what keeps them out. Five controls (an unmutated copy of each of the three adapters passes; an adapter omitting the optional `create_target`, or `link_pattern`, passes) pin the other direction, and every mutation is verified to have actually changed the file, so an anchor that rotted away cannot masquerade as a detection. The argument is sharper here than for the two checkers above: the only adapters in this tree are conformant ones, so the gate passes on every run whether or not it still works.
- `./scripts/docs-adapter-assert.sh` — the Confluence docs adapter's output, offline: the real adapter against a stub `curl` serving canned pages, asserting the Markdown conversion, section cuts (by `--section` and by URL fragment), the truncation cap and flag, `doc-stat`'s shape, excerpt cleaning and the CQL `doc-search` sends, the credential and site fallbacks, the foreign-site refusal, and that the API token never reaches `curl`'s command line. It proves the transformation; only a live call proves the canned payloads still match Confluence.
