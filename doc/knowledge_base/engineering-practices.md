...
- `./scripts/roadmap-length-check.sh` — every item in `doc/knowledge_base/roadmap.md`, pending and completed alike, fits in 200 characters. The roadmap is loaded by every session, so an over-long item is a cost paid forever; the cap had been convention-only and 46 of 94 items had drifted past it. Scoped to that one file — the template's and demo seed's roadmaps are instructional prose and legitimately longer. Carries an inline self-check (three fixtures either side of the cap, one multibyte) so a matcher that stopped matching cannot report clean. See ADR 0005.
