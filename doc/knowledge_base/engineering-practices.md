...
- `./scripts/prompt-seam-assert.sh` — the seam checker's own contract, by mutation: each of its nine checks is required to *fail* on a tree where that one seam is broken, in a throwaway copy. Two controls (an unmutated copy passes; a whitelisted dangling reference is silenced) keep the nine honest. A guard that only ever passes is indistinguishable from one that stopped working.
