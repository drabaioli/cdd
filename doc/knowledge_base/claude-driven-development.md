...
Two capabilities are live: the **tracker**, with a GitHub reference adapter and a Jira adapter, and **docs**, with a Confluence reference adapter; one conformance gate covers both, and every other binding is still the built-in one. The docs capability is **read-only** and on the *mirror* side of the rule above: a session consults it for context the repo does not hold — an integration contract, a platform spec — never as a source for the repo's own docs. The sessions that gather context may read through it, but none does by default: it is called only on a trigger (a page reference in the task, a project note saying what lives in the store, or a task that depends on an external system the repo does not document), and what it returns is capped, so one lookup cannot flood a session's context. A project without one pays nothing — no call, no line. The project note behind the second trigger is asked for when CDD is set up — during bootstrap discovery, and at retrofit — and lives as one line in `CLAUDE.md`. The decision and its reasoning are recorded in `doc/architecture/adr/0007-extend-cdd-through-capability-adapters.md`; the verb contracts, JSON shapes and exit codes are pinned in `doc/architecture/capability-adapters.md`, which is what an adapter author reads.
...
Output is a pass/fail/skipped summary across the gates. The session then auto-commits its reconciliation edits locally per §2.11, and ends with an optional, human-gated step to open the PR: a single yes/no question; on approval it derives the title and body and runs `gh pr create`, adding one close line per issue reference recorded on the task's state record (§2.13). The line itself comes from the resolved tracker (§2.16), which is what makes the close work on a backend whose syntax is not GitHub's; a backend that declares no way to phrase one yields no close lines, said once rather than guessed. A close line is a string, not a guarantee: the issue closes on merge where the tracker and the forge are the same backend, and otherwise only where the tracker's own forge integration is installed — so the session says which case applies before it asks, rather than letting an inert line look like a working one. The three cases, and what a guaranteed cross-backend close would need, are in `doc/architecture/capability-adapters.md`. With no usable record there are no close lines at all, which the session says at the confirmation step rather than opening a silently incomplete PR. `/cdd-pre-pr` never opens a PR without explicit confirmation.
