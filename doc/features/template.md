...
## The `/quick-create` command

A CDD-repo-only slash command (`.claude/commands/quick-create.md`, deliberately not shipped in the template) for producing a small, self-contained deliverable — a script plus a README — without any of the project substrate (no roadmap, no `doc/` tree, no worktree helper, no baseline marker, no `/pre-pr` lifecycle). It is the third sibling of `/bootstrap` and `/retrofit`, but unlike them it needs **neither** `template/` nor the bootstrap script: a one-off has no template, so it writes plain files directly into the target. Run from a CDD-repo session with an optional one-line description, it is a **guided but deliberately lighter** session: a scope check against the shared deliverable-or-project heuristic (with an off-ramp to `/bootstrap` if project-signals trip), a few discovery questions (what / goal / non-goals), then a files-first write of the artifact(s) and a focused README into the target (default: a sibling of the CDD repo). A smoke test, a local commit, and a GitHub repo are each offered separately and never done by default. The reverse off-ramp lives in `/bootstrap`, which offers to drop to `/quick-create` when discovery reveals a trivial single artifact. If a deliverable later grows into a project, `/retrofit` can install CDD onto it.
