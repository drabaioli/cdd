...
**`seams` — `scripts/prompt-seam-check.sh`.** Deterministic seam-contract checks over the repo's own prompts, guarding against a one-sided edit silently stranding a downstream prompt-driven step. It verifies 10 seams with grep only (no LLM, no API key): every `/cdd-*` reference across the repo's markdown resolves to an existing command file (known non-commands are whitelisted in `scripts/prompt-seam-whitelist.txt`); the issue references `cdd-next-step.md` records with `cdd-state issue-refs` are still read back with `cdd-state get issue_refs` and turned into close lines through `issue-close-token` in `cdd-pre-pr.md`, and the legacy `gh_issue_NN` branch token is still produced there and still consumed as the `Closes #NN` fallback; backticked file paths in the command files, `CLAUDE.md`, `README.md` and the process doc resolve to real files; each `cdd-*.md` still carries its load-bearing headings, matched by title so that renumbering a step is a no-op while a dropped heading still fails; the gate count stated in prose here and in `CLAUDE.md` matches what `./scripts/ci.sh list` registers; every open row of the template's engineering-practices contract is named in the engineering-floor question `/cdd-bootstrap` asks, so a practice added there cannot ship resolved by guess; every `## ` section `cdd-plan.md` writes into the plan file is still named in `cdd-implement.md`, which reads it; the small-change lane's routing marker is still written by `cdd-next-step.md` and still read by `tools/cdd-worktree.sh` on both the launch and the resume path; the lane's one-sentence eligibility heuristic is stated verbatim in the process doc and in both commands that apply it; and the number of seam checks stated in prose here and in three other files matches the checker's own `CHECKS` registry, which `./scripts/prompt-seam-check.sh list` prints.

Note that the issue-ref/branch-token check reads this file with its `cdd-only` regions **stripped**: the paragraph above quotes all six of that check's tokens while describing it, so grepping the raw file would let the §11 close-line block be deleted outright and still report clean. A needle satisfied only by this section does not count.
...
- **Empty** (no record, an unsynced or reaped one, or no `jq`): fall back to the branch name. If it matches `gh_issue_NN` (e.g. `gh_issue_42_dark_mode`), parse `NN` and append `Closes #NN`. This path is deliberately not routed through an adapter — it is today's behaviour byte for byte, and it is what keeps an unusable record from silently losing the close line.
